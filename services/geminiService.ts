...
export const generateMakerGuide = async (
  ...
  systemInstruction += "\n\nSTRICT OUTPUT STRUCTURE:\n" +
    "1. **Hardware Required**: Create a distinct section listing every component needed. Include specific part numbers (e.g., 'SSD1306', 'MPU6050', '10kΩ Resistor', 'Breadboard', 'Jumper Wires').\n" +
    "2. **Wiring Diagram**: Clear pin-to-pin connection instructions (e.g., 'Sensor VCC -> Board 5V').\n" +
    "3. **Code Implementation**: The complete, working code.\n" +
    "4. **Explanation**: Brief summary of operation.";

  // Language preference handling
  if (languagePreference === 'micropython') {
    systemInstruction += " \n\nSTRICT CODE REQUIREMENT: You MUST provide the code examples in MicroPython. \n" +
    ...
    systemInstruction += " \n\nSTRICT CODE REQUIREMENT: You MUST provide the code examples in C++ (Arduino Framework/PlatformIO). \n" +
    ...
    systemInstruction += " \n\nSTRICT CODE REQUIREMENT: You MUST provide the code examples in CircuitPython. \n" +
    ...
    // Default behavior
    systemInstruction += " \n\nProvide complete, well-commented code examples. Use Python for Raspberry Pi/Pico, C++ for Arduino/ESP32/M5Stick, and MakeCode (JavaScript/Blocks description) or MicroPython for Micro:bit.";
  }
  ...
  switch (type) {
    case 'sensor':
      prompt = `Create a step-by-step tutorial for connecting and using a "${topic}" with a ${board}.`;
      break;
    case 'game':
    ...
    case 'barcode':
      ...
      prompt = `Create a comprehensive guide to build a barcode reader using a ${board}. The user is interested in using: ${topic}.`;
      break;
    case 'network':
